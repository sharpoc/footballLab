...
- Confirmed lineup Task 1–7 的本地离线链路已完成 post-breaker targeted follow-up：六联赛独立单实例 pre-match runner 继续持久绑定每个 receipt 的发布前/后首选与 component/aggregate snapshot，pending-first 优先收敛 Task 5 committed/published，Task 6 故障不阻断 Task 5/Task 4。Task 5 validator 仅接受真实可达的“ingest `stored/duplicate` 已发生、ACK state commit 失败”专用形状：顶层必须为 `publish_failed`，该 reason 只能位于 retryable，且 token 必须精确等于 Task 5 state 中与本轮 publication 匹配的 current receipts；正常失败保留 `committed`，原子 replace 已生效但后续 commit 抛错时也允许 component/aggregate/publish status 全匹配的 `published`。current receipt 联赛是完整 aggregate components 的子集；aggregate 必须精确等于正式 acceptance active 集合，且所有 component 都与 Task 5 已提交分区 cache 的 snapshot ID/联赛声明一致，因而允许合法 active cached extras，同时拒绝缺 active、non-active extra 和 cache 证据错配。合法形状仍不绑定、不通知、不清 current receipt，并继续本轮 Task 4 polling/其他 due。任何 `partial` / `published` / `already_acked` / `blocked` / 其他 status、失败或缺失 publication、blocked 组 reason、旧 persisted/missing/snapshot 不匹配/不完整 current membership 均在绑定、通知和清理前 fail closed。Source episode 的 generation、exact fingerprint 与 recovery supersede 因果契约保持不变。最新验证：Task 7 focused `38/38`、Task 1–7 focused `154/154`、完整回归 `1324/1324 tests passed, 1 optional fastapi module skipped`；CLI dry-run 与 compile/diff/sensitive 验证在本轮最终提交前重新执行。本阶段未联网、未读真实 `.env`、未消耗 quota、未真实发布/通知、未安装 timer、未 push/merge/deploy。
