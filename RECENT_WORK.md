...
- 展示的封盘首选只从开赛前最后一刻仍有效的 `match_decision` 做公开字段投影；开赛前已过期或记录不完整的首选不会被恢复。本轮不改模型、首选方向、公开 API、snapshot 契约、采集、调度或数据库。
- TDD 新增“已开赛但未确认完赛”回归用例，先确认旧逻辑红灯，实现后配置运行时完整回归 `717/717`、系统 Python FastAPI `13/13` 通过；`py_compile` 和 `git diff --check` 通过。
...
- 改动限定在 `worldcup.ledger` 投影层、`worldcup.ledger_html` 渲染层和 `tests/test_ledger.py` / `tests/test_preview.py` 回归断言；未改调度策略、refresh plan 计算、采集、The Odds API 调用、snapshot schema 或线上写入逻辑。
- 验证：新增红灯断言先确认详情卡旧逻辑缺少紧凑日期；实现后 `54/54 ledger+preview tests passed`，`py_compile worldcup/ledger.py worldcup/ledger_html.py` 和 `git diff --check` 通过。
