...
- Confirmed lineup Task 1–7 的本地离线链路已完成第四轮 review follow-up：六联赛独立单实例 pre-match runner 继续持久绑定每个 receipt 的发布前/后首选与 component/aggregate snapshot，pending-first 优先收敛 Task 5 committed/published，Task 6 故障不阻断 Task 5/Task 4。Task 5 validator 现接受真实可达的“ingest `stored/duplicate` 已发生、ACK state commit 失败”形状，但只投影为顶层 `publish_failed + retryable=ack_state_commit_failed`：不绑定、不通知、不清 receipt，并继续本轮 Task 4 polling/其他 due；嵌套伪成功、全 durable failure 或 persisted ACK 翻转仍 fail closed。Source episode 新增单调 generation 与 exact failure/recovery notification fingerprint；本轮 event evidence 会在 Task 6 retry 前提交，canonical pending 不再等同送达，recovery 只有 `sent/already_sent` 才关闭。relapse 会 supersede 未发送 recovery，并在 Task 6 自身锁下精确删除对应 canonical pending，避免 `failure1 -> failure2 -> recovery1`；未达门槛恢复、event 归因、T-20 active/SCHEDULED 门禁及改期稳定身份契约保持不变。最新验证：Task 7 focused `36/36`、Task 1–7 focused `152/152`、完整回归 `1322/1322 tests passed, 1 optional fastapi module skipped`，`py_compile`、`git diff --check` 和高信号敏感值扫描通过；CLI dry-run 返回 `dry_run / invalid_acceptance_report`，leagues cache/local 内容+mtime 指纹前后同为 `0c8417c766c27c39bebe93f3a345402bf69331be4d17a6ad0a107c3c8b0c73ea`。本阶段未联网、未读真实 `.env`、未消耗 quota、未真实发布/通知、未安装 timer、未 push/merge/deploy。
