...
- Confirmed lineup Task 1–7 的本地离线链路已收口：新增六联赛独立单实例 pre-match runner，持久每个 receipt 的发布前/后首选与 component/aggregate snapshot 绑定，pending-first 优先收敛 Task 5 committed/published，Task 6 pending 只在 Task 5/Task 4 业务推进后独立重试。三轮 review follow-up 已修复通知队列阻断发布、同轮多 snapshot 错绑、source episode 跨比赛扩散、failure/recovery 持久与顺序、relapse/改期重复 failure、inactive/terminal T-20 误通知及 Task 4/5/6 伪成功返回。已绑定 receipt、达门槛 failure 和 exact recovery `succeeded` 现在都会先成为可跨进程重建的 durable notification intent；未达门槛恢复静默关闭，failure 未实际 sent 前不会越过发 recovery，relapse 会取消 recovery 并保持 active。Task 5 已用完整可达矩阵校验顶层 status/ACK/publication；失败顶层不得被嵌套伪成功、全 durable 或 persisted ACK 翻转，合法 `already_acked` 和 mixed `partial` 仍可收敛。T-20 只接受完整 lineup state 和明确 `SCHEDULED`，聚合 source count、损坏 state 和未知 status 均 fail closed。新 LaunchAgent generator 仍默认 observation mode，固定独立 label 和 300 秒间隔，只生成 JSON/plist，不调用 `launchctl`。Task 7 focused `34/34`、Task 1–7 focused `150/150`、完整回归 `1320/1320 tests passed, 1 optional fastapi module skipped`，`py_compile` 和 `git diff --check` 已通过；当前空 leagues cache/local 的 CLI dry-run 前后内容+mtime 指纹均为 `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`。本阶段未联网、未读真实 `.env`、未消耗 quota、未真实发布/通知、未安装 timer、未 push/merge/deploy。
