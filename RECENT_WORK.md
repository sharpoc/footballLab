...
- 通知区分“尚未获取正式首发”和“已获取正式首发”；缺失时明确未完成首发后复核、旧推荐及赔率有效性未验证，不展示旧概率/赔率。既有事件指纹不变，保留去重、失败重试及方向未变时的首发后更新。
- TDD 已复现原有跳过和文案问题；新增真实临时 snapshot/outbox 重启去重、失败重试后补发 unchanged 更新回归。完整离线回归 1505/1505 通过，1 个可选 fastapi 模块跳过。
- 仅在独立 worktree 修改源码/测试/文档；未真实请求 provider、未消耗额度、未发送 WxPusher、未修改运行目录或定时器，未提交/推送/部署。首发真实解析修复仍等待独立 probe 证据。
