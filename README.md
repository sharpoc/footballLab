...
- 每 15 分钟 sidecar wave 的候选 discovery 与 selection 共用同一 `Asia/Shanghai` `[18:00, 次日18:00)` 半开 cycle；当前 wave 无 due request 时，会读取同 cycle 旧 snapshot 的 event rows，先清除已开赛、过期、延期/取消、改期 identity、cycle 外或不再满足 selection gate 的候选，再按稳定 event identity 合并本 wave 新 rows 并重新计算 Top4、2串1、3串1，不直接复制旧榜单。provider 解析/身份/市场失败或空响应不会复活对应旧候选；跨 cycle 不复用。
...
- production sidecar 入口为 `python3 -m worldcup.daily_sidecar`：默认 `--dry-run`，只有显式 `--live` 才读取 `THE_ODDS_API_KEY_PRIMARY` / `THE_ODDS_API_KEY_SECONDARY` / `THE_ODDS_API_KEY_TERTIARY`，先做 quota guard，再调用 `/sports`、`/events` 和按 anchor 的 `/odds`。生产数据目录通过 `WORLDCUP_DAILY_ODDS_DATA_DIR` 或 `--data-dir` 显式配置，ECS 使用 `/var/lib/worldcup/daily_odds`，HTTP reader 读取同一路径；snapshot/state 采用原子替换与独占锁。daily budget 默认且上限为 85 credits，失败时 fail-closed，不输出 key。
- Git 管理的 `deploy/systemd/worldcup-daily-sidecar.service` / `.timer` 由代码部署安装并 `daemon-reload`，但部署阶段明确保持 timer disabled；只有 live refresh、snapshot/state、权限、readyz 与两个 sidecar 公网路由验收通过后才允许人工执行 `systemctl enable --now worldcup-daily-sidecar.timer`。该链路不修改旧 `/api/daily-picks`、旧单场 refresh/scheduler 或 LaunchAgent。
